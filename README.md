...
- 店主访问 `/admin` 的“发布静态版”生成 GitHub Pages 可用的 `docs` 目录
...
如果客户只看线上相册，建议使用 GitHub Pages：

1. 在本地后台上传、整理照片。
2. 后台点击“发布静态版”，生成 `docs` 目录。
3. 双击 `publish_to_github.bat`，把代码和 `docs` 推送到 GitHub。
   如果 bat 窗口仍然报路径错误，请右键 `publish_to_github.ps1`，选择“使用 PowerShell 运行”。
4. 在 GitHub 仓库设置里启用 Pages：
   - Source: Deploy from a branch
   - Branch: `main`
   - Folder: `/docs`
5. 自定义域名填写：`www.youzizhijia.dpdns.org`

以后每次更新照片后，重复“发布静态版”并运行 `publish_to_github.bat`，客户网站就会跟着更新。
